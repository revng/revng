#ifndef _REVNG_ASSERT_H
#define _REVNG_ASSERT_H
...
noret void revng_check_fail(const char *CheckBody,
                            ...
                            const char *File,
                            unsigned Line);
noret void revng_do_abort(const char *Message,
                          const char *File,
                          unsigned Line);
...
/// Use this macro to ensure program termination in case of an unexpected
/// situation.
#define revng_check(what, message) do {         \
    bool Condition = boolcast(what);            \
    if (!Condition) {                           \
      revng_check_fail(#what,                   \
                       message,                 \
                       __FILE__,                \
                       __LINE__);               \
    }                                           \
...
/// certain situation will never happen at run-time, which might open up to new
/// optimization opportunities.
#define revng_assert(what, message) do {        \
    bool Condition = boolcast(what);            \
    if (!Condition) {                           \
      revng_assert_fail(#what,                  \
                        message,                \
                        __FILE__,               \
                        __LINE__);              \
    }                                           \
