...
# WIP REFACTOR: decide if we want to make a class out of this. If we do, use DedupMeta.
...
class Model(ABC):
    ...
    def all_objects(self, kind: type[ObjectID]) -> ObjectSet:
        """
        Returns all the objects of a certain kind in the model.
        """
        assert ObjectID.ROOT is not None and isinstance(
            ObjectID.ROOT, type
        ), "ObjectID.ROOT must be defined before calling all_objects and it must be a type."
        root = ObjectID.ROOT()  # pylint: disable=not-callable
        return ObjectSet(kind, self.children(root, kind).objects)

    # WIP LOW: test
    def move_to_kind(self, objects: ObjectSet, destination_kind: type[ObjectID]) -> ObjectSet:
        if not objects:
        ...
        match destination_kind.relation(source_kind):
            case (ObjectID.Relation.SAME, _):
                return objects.clone()
            case (ObjectID.Relation.UNRELATED, _):
                return ObjectSet(destination_kind)
            case (ObjectID.Relation.ANCESTOR, path):
                ...
                for obj in objects.objects:
                    ...
                    result.add(obj)
                return result
            case (ObjectID.Relation.DESCENDANT, path):
                assert isinstance(path, list)
...
# WIP REFACTOR: maybe we could to this just with some typing magic?
