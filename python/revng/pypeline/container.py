...
from typing import Annotated, Dict, Generator, Generic, Optional, Tuple, Type, TypeVar
...
class ContainerDeclaration(Generic[C]):
    """
...
def load_container(
    ...
    with open(path, "r", encoding="utf-8") as f:
        container.deserialize(
            {ObjectID.deserialize_any(k): bytes.fromhex(v) for k, v in json.load(f).items()}
        )
    return container
...
# WIP REFACTOR: a Container holds a reference to the declaration. Switch to Set[Container]?
