...
from .storage.storage_provider import SavePointsRange, StorageProvider, StorageProviderFileStorage
...
class PipelineNode:
    ...
    def run(
        ...
        if isinstance(self.task, SavePoint):
            ...
            deps = self.task.run(
                file_storage=StorageProviderFileStorage(storage_provider),
                model=model,
