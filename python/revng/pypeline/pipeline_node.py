...
from revng.pypeline.container import Configuration, ConfigurationId, ContainerDeclaration
...
def deterministic_sort_key(node: PipelineNode) -> SupportsRichComparison:
    ...
    """
    return (
        tuple(node.arguments),  # Arguments of the task
        ...
        node.task.name,  # Name of the task
    )
...
class PipelineNode:
    ...
    def run(
        ...
        if isinstance(self.task, SavePoint):
            ...
            return None  # SavePoints do not return any dependencies
        elif isinstance(self.task, Pipe):
            ...
            return [(self.bindings[index].name, obj, path) for (index, obj, path) in deps]
        else:
            raise TypeError(f"Unsupported task type: {type(self.task)}")
