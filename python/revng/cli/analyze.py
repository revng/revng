...
class AnalyzeCommand(Command):
    def __init__(self):
        super().__init__(("analyze",), "revng analyzer", False)
    ...
    def run(self, options: Options):
        ...
        command = build_command_with_loads(
            "revng-analyze", pipelines_args + options.remaining_args, options
        )
        return run(command, options)


commands_registry.register_command(AnalyzeCommand())
