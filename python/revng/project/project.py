...
class Project(ABC):
    ...
    def __init__(self):
        ...
        self.pipeline_description: PipelineDescription = self._get_pipeline_description()
        ...
        self._analysis_names = list(
            chain.from_iterable(
                [an.Name for an in st.Analyses] for st in self.pipeline_description.Steps
            )
        )
        self._analyses_list_names = [x.Name for x in self.pipeline_description.AnalysesLists]
    ...
    def _get_artifact(
        ...
        kind_name = self.pipeline_description.Steps[artifact_name].Artifacts.Kind
        rank_name = self.pipeline_description.Kinds[kind_name].Rank
        rank = self.pipeline_description.Ranks[rank_name]
    ...
    def _get_artifact_container(self, artifact_name: str) -> Container:
        artifact_container = self.pipeline_description.Steps[artifact_name].Artifacts.Container
        container = self.pipeline_description.Containers[artifact_container]
    ...
    def _compute_rank_matchers(self) -> Dict[str, re.Pattern]:
        result = {}
        for rank in self.pipeline_description.Ranks:
...
class CLIProjectMixin:
    ...
    def __init__(self, revng_executable_path: Optional[str]):
        ...
        self.revng_executable_path = _revng_executable_path

    def _run_revng_cli(self, cmd_args: Iterable[str], *args, **kwargs):
        try:
            return run([self.revng_executable_path, *cmd_args], *args, **kwargs, check=True)
