...
def registry(
    ...
    def wrap(cls) -> type:
        ...
        # any arguments so we need to check if the original method is a builtin and
        # replace it with a no-op.
        if inspect.isbuiltin(cls.__init_subclass__):
            original_init_subclass = classmethod(lambda *a, **kw: None)
