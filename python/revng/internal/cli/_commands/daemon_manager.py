...
# clients.
# Each clients registers to the daemon declares when it's alive by
...
class ClientError(Exception):
    """The client is not registered, or it is already registered."""
    ...
    async def _run(self):
        while not self._stopping:
            with NamedTemporaryFile() as temp_file:
                ...
                if return_code != 0:
                    cli_logger.debug_log(f"Daemon on {self._path} exited abruptly with {return_code}")
                self._ready.clear()
                self._process = None
...
def make_starlette(production: bool, manager: DaemonManager) -> Starlette:
    ...
    return Starlette(
        ...
        exception_handlers={ClientError: lambda req, exc: Response(status_code=400)},
    )
