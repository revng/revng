...
public:
  ...
  static llvm::Error concatenate(llvm::Error SourceError,
                                 llvm::StringRef NewReason,
                                 std::unique_ptr<LocationBase> Loc) {

    if (not SourceError.isA<DocumentErrorBase>())
      return SourceError;

    llvm::Error ToReturn = llvm::Error::success();
    return llvm::handleErrors(std::move(SourceError),
                              [&](std::unique_ptr<revng::DocumentErrorBase>
                                    DocumentError) {
                                DocumentError->Entries
                                  .emplace_back(NewReason.str(),
                                                std::move(Loc));
                                return llvm::Error(std::move(DocumentError));
                              });
  }
