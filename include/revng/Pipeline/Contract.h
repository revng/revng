...
enum class InputPreservation { Erase, Preserve };
...
///
/// A contract from root(granulartiy 1) to function(granularity 2), requires
/// that the pipe it is attached to will convert all roots with pathComponents
...
public:
  constexpr Contract(const Kind &Source,
                     ...
                     InputPreservation Preserve = InputPreservation::Erase) :
    Source(&Source),
  ...
  constexpr Contract(const Kind &Target, size_t PipeArgumentTargetIndex = 0) :
    ...
    PipeArgumentTargetIndex(PipeArgumentTargetIndex),
    Preservation(InputPreservation::Erase) {}
...
private:
  ...
  Exactness backwardInputContract(const Target &Output) const;
  ...
  /// target is the container in which the Pipe would write when used to
  /// produce the targets
...
/// All subcontracts inside a group contract are evaluated in parallel
/// on the same input and their, and their output is merged.
class ContractGroup {
public:
  ...
  ContractGroup(const Kind &Source,
                ...
                const Kind &Target,
                size_t PipeArgumentTargetIndex = 0,
                InputPreservation Preservation = InputPreservation::Erase) :
