...
/// run it. it is the top level object on which to invoke operations.
class Runner {
private:
...
private:
  constexpr static const char *Message = "you can only registers containers "
                                         "before adding a step, otherwise the "
                                         "already present steps will not be "
                                         "aware of the newly registered "
                                         "containers.";
...
public:
  ...
  /// Prefer this overload when the container you wish to add
  /// is default constructible. This should be most of the time,
  /// but sometimes this is not possible, in particular when some context
  /// information is needed such as llvm containers that require the llvm
  /// context to be constructed.
  ...
  /// Given a target, all occurrences of that target from every
  /// container in every step will be registered in the returned invalidation
  /// map. The propagations will not be calculated.
  llvm::Error
  getInvalidations(const Target &Target, InvalidationMap &Invalidations) const;

  /// Deduces and register in the invalidation map all the targets
  /// that have been produced starting from targets already presents in the map.
  llvm::Error getInvalidations(InvalidationMap &Invalidated) const;

  /// remove the provided target from all containers in all the steps,
  /// as well as all all their transitive dependencies
