...
///
/// This class is used mostly in two points, as the backing containers
/// of each step, as well as the containers containing the target extracted
/// from each step to perform the computation.
///
  ...
  void dump(OStream &OS, size_t Indents = 0) const {
    for (const auto &Entry : Content) {
      ...
      if (Entry.second != nullptr)
        Entry.second->enumerate().dump(OS, Indents);
    }
  }
