...
struct RecursivePromise : public ReturnBase<ReturnT> {
  ...
  using CoroHandleType = std::coroutine_handle<RecursivePromise>;

  template<typename>
  friend struct RecursivePromise;

  RecursiveCoroutine<ReturnT> get_return_object() {
    return RecursiveCoroutine<ReturnT>(CoroHandleType::from_promise(*this));
  }
  ...
  // Not copyable, otherwise Continuation could be resumed twice.
  ...
  auto await_transform(RecursiveCoroutine<AwaiteeReturnT> &&Awaitee) {
    ...
    using AwaiteeCoroHandle = typename AwaiteePromiseT::CoroHandleType;
...
public:
  ...
  RecursiveCoroutine(CoroHandleType H) : OwnedHandle(H) {}
...
private:
  ...
  static void runCoroutine(CoroHandleType Handle) {
    revng_assert(not Handle.done());
    Handle.resume();
    revng_assert(Handle.done());
  }

  CoroHandleType OwnedHandle;
};
