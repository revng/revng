...
/// are adhered to.
///
/// \note this also correctly handles `llvm::Error` and `llvm::Expected`
/// when they are passed into \param IfNull.
...
ReturnT upcast(P &&Upcastable, const L &Callable, ReturnT &&IfNull) {
  ...
  if constexpr (I < std::tuple_size_v<concrete_types>) {
    ...
    if (auto *Upcasted = llvm::dyn_cast<type>(Pointer)) {
      ...
      if constexpr (SpecializationOf<std::decay_t<ReturnT>, llvm::Expected>)
        llvm::consumeError(std::move(IfNull).takeError());
