...
public:
  std::set<pypeline::ModelPath> diff(const Model &Other) const {
    auto Diff = ::diff(*TheModel.get(), *Other.TheModel.get());
    std::set<pypeline::ModelPath> Result;
  ...
  std::set<ObjectID> children(const ObjectID &Obj, ObjectID::Kind Kind) const {
    if (Obj.kind() == ObjectID::Kind::Root
        && Kind == ObjectID::Kind::Function) {
    ...
    if (Obj.kind() == ObjectID::Kind::Root
        && Kind == ObjectID::Kind::TypeDefinition) {
      std::set<ObjectID> Result;
  ...
  pypeline::Buffer serialize() const {
    pypeline::Buffer Out;
