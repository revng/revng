...
public:
  ...
  void registerContainerFactory(llvm::StringRef Name, Args &&...A) {
    KnownContainerTypes.try_emplace(
        Name, [&A...]() { return std::make_unique<ContainerFactory>(A...); });
  }
  ...
  template <typename EnforcerType> void registerEnforcer(llvm::StringRef Name) {
...
private:
  llvm::Error parseStepDeclaration(PipelineRunner &Runner,
                                   const StepDeclaration &) const;
  llvm::Error parseInvocation(Step &Step,
  ...
  llvm::Error parseLLVMPass(Step &Step,
                            const EnforcerInvocation &Invocation) const;
...
template <> struct llvm::yaml::MappingTraits<AutoEnforcer::EnforcerInvocation> {
...
template <> struct llvm::yaml::MappingTraits<AutoEnforcer::StepDeclaration> {
