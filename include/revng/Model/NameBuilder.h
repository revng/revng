...
public:
  ...
  protected:
    ...
    nameImpl(const SortedVector<MetaAddress> &UserLocationSet,
             RangeOf<const model::LocalIdentifier &> auto KnownNames,
      ...
      bool IsLocationUnique = EmittedLocations.emplace(UserLocationSet).second;
      ...
      auto [_, Inserted] = EmittedNames.emplace(Iterator->Name());
      if (not Inserted) {
      ...
      if (llvm::Error Reason = Parent->isNameReserved(Iterator->Name())) {
        ...
        std::uint64_t CurrentIndex = NextIndex++;
  ...
  struct VariableNameBuilder : public CountingNameBuilder {
    ...
    auto name(const SortedVector<MetaAddress> &UserLocationSet) {
