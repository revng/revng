...
public:
  ...
  private:
    ...
    llvm::StringSet<> EmittedLocations = {};

  protected:
    const NameBuilder &parent() const { return revng::assertNonNull(Parent); }
  ...
  protected:
    ...
    nameImpl(RangeOf<MetaAddress> auto const &UserLocationSet,
      ...
      bool IsLocationUnique = EmittedLocations.insert(std::move(AsStr)).second;
  ...
  public:
    ...
    const model::Function &function() const {
      return revng::assertNonNull(Function);
    }

    CountingNameBuilder::NamingResult
    name(RangeOf<MetaAddress> auto const &UserLocationSet) {
