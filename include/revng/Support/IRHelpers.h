...
/// \brief Replace all uses of \ToReplace, in many functions at once.
///
/// \return true if it changes something, false otherwise.
///
/// \Replacement is a map, where the key are the Functions where
/// the substitution have to take place, and the mapped value is the Value that
/// has to be substituted in \ToReplace uses.
using FunctionToValueMap = llvm::ValueMap<llvm::Function *, llvm::Value *>;
inline bool
replaceAllUsesInFunctionsWith(llvm::Value *ToReplace,
                              const FunctionToValueMap &Replacement) {

  bool Changed = false;

  llvm::SmallPtrSet<llvm::ConstantExpr *, 8> ConstExprCSVCasts;

  auto UI = ToReplace->use_begin();
  auto E = ToReplace->use_end();
  for (; UI != E;) {
    llvm::Use &U = *UI;
    ++UI;
    auto *I = dyn_cast<llvm::Instruction>(U.getUser());

    if (I != nullptr and I->getParent() != nullptr) {
      auto It = Replacement.find(I->getFunction());
      if (It != Replacement.end()) {
        revng_assert(It->second != nullptr);
        U.set(It->second);
        Changed = true;
      }
    } else if (auto *CE = dyn_cast<llvm::ConstantExpr>(U.getUser())) {
      // We can't convert ConstantExprs to Instructions while iterating on
      // ToReplace uses. This would mess with them creating new uses of
      // ToReplace in the Instructions generated by converting the
      // ConstantExprs. We store them in a helper vector and process them later.
      if (CE->isCast())
        ConstExprCSVCasts.insert(CE);
    }
  }

  for (llvm::ConstantExpr *CE : ConstExprCSVCasts) {
    // We iterate on the uses of each ConstantExpr while generating
    // instructions that use ToReplace. This is safe because we don't change
    // ToReplace
    // uses while iterating on them.
    auto CEIt = CE->use_begin();
    auto CEEnd = CE->use_end();
    for (; CEIt != CEEnd;) {
      llvm::Use &CEUse = *CEIt;
      ++CEIt;
      auto *CEInstrUser = dyn_cast<llvm::Instruction>(CEUse.getUser());
      if (CEInstrUser and CEInstrUser->getFunction()) {
        auto *F = CEInstrUser->getFunction();
        auto It = Replacement.find(F);
        if (It != Replacement.end()) {
          auto *Cast = CE->getAsInstruction();
          revng_assert(It->second != nullptr);
          Cast->replaceUsesOfWith(ToReplace, It->second);
          Cast->insertBefore(CEInstrUser);
          CEUse.set(Cast);
          Changed = true;
        }
      }
