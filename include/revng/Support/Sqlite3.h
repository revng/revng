...
public:
  // Bind BLOB type
  void bind(int Position, llvm::ArrayRef<const char> Array) {
  ...
  void execute() {
    ...
    revng_assert(Result == SQLITE_OK || Result == SQLITE_DONE);
  }
  ...
  cppcoro::generator<std::tuple<T...>> execute() {
    while (true) {
      int Result = sqlite3_step(Statement.get());
      revng_assert(Result == SQLITE_ROW || Result == SQLITE_DONE);
