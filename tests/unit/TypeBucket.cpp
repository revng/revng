...
BOOST_AUTO_TEST_CASE(Paths) {
  ...
  model::UpcastableType Saved;
  {
    ...
    revng_check(Binary->TypeDefinitions().size() == 0);
    // Not valid yet.
    // revng_check(Type->asDefinition().OriginalName() == "Valid");
    Bucket.commit();
    revng_check(Binary->TypeDefinitions().size() == 1);
    ...
    Saved = Type.copy();
    revng_check(Saved->asDefinition().OriginalName() == "Valid");
  ...
  revng_check(Saved->asDefinition().OriginalName() == (*TD)->OriginalName());
  revng_check(Saved->asDefinition().OriginalName() == "Valid");
}
