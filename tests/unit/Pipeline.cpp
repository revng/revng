...
BOOST_AUTO_TEST_CASE(LoaderTestFromYamlLLVM) {
  ...
  Loader.registerLLVMPass<LLVMPassFunctionCreator>("CreateFunct"
                                                   "ionPa"
                                                   "ss");
  Loader.registerLLVMPass<LLVMPassFunctionIdentity>("IdentityPa"
                                                    "ss");
...
class EnumeratableContainerExample
  : public EnumeratableContainer<EnumeratableContainerExample> {
public:
  static char ID;

  EnumeratableContainerExample(Context &Ctx, llvm::StringRef Name) :
    EnumeratableContainer<EnumeratableContainerExample>(Ctx, Name) {}

  unique_ptr<ContainerBase>
  cloneFiltered(const TargetsList &Container) const final {
    return make_unique<EnumeratableContainerExample>(*this);
  }

  ~EnumeratableContainerExample() override = default;
...
private:
  void mergeBackImpl(EnumeratableContainerExample &&Container) override {}
};

char EnumeratableContainerExample::ID;

class ExampleContainerInpsector
  : public ContainerEnumerator<EnumeratableContainerExample> {
public:
  ExampleContainerInpsector() :
    ContainerEnumerator<EnumeratableContainerExample>(RootKind) {}
  TargetsList
  enumerate(const Context &Ctx,
            const EnumeratableContainerExample &Container) const final {
    TargetsList ToReturn;
  ...
  bool contains(const Target &Target,
                const EnumeratableContainerExample &Container) const {
    return Container.Targets.count(Target);
  }

  bool remove(const Context &Ctx,
              const TargetsList &Targets,
              EnumeratableContainerExample &Container) const {
  ...
  bool
  remove(const Target &Target, EnumeratableContainerExample &Container) const {
...
BOOST_AUTO_TEST_CASE(EnumeratableContainersTest) {
  Context Ctx;
  EnumeratableContainerExample Example(Ctx, "dont_care");
