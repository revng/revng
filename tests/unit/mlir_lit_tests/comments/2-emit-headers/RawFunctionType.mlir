...
!rawfunction_0_ = !clift.func<
  ...
  return_value_comment "Even though this function only returns one register, who's to say that\0Aregister is not interesting enough to write a really long essay about it?!\0A\0AWith multiple line breaks,\0A\0Aand non-trivial formatting too!\0A```cpp\0A  // Meta comment: comment within a comment\0A  // ```cpp\0A  //   // Could be within another comment too!!!\0A  // ```\0A```\0A\0ALet's see how well this will be handled!!"
>
module attributes {clift.module} {
  clift.func @my_commented_function<!rawfunction_0_>(
    !uint64_t {
      clift.c_attributes = [#clift.c_attribute<"_REG" : "/macro/_REG"(identifier "rax_x86_64" : "/macro-argument/_REG/rax_x86_64")>],
