...
// RUN: ../%revngpipe import-model-names %S/../0-import-types/RawFunctionType.yml %s /dev/stdout | ../%revngcliftopt | FileCheck %s
...
// CHECK:   [
// CHECK:     #clift.c_attribute<"_ABI" : "/macro/_ABI"(identifier "raw_x86_64" : "/macro-argument/_ABI/raw_x86_64")>
// CHECK:   ]
...
// CHECK:   return_value_comment "Even though this function only returns one register, who's to say that\0Aregister is not interesting enough to write a really long essay about it?!\0A\0AWith multiple line breaks,\0A\0Aand non-trivial formatting too!\0A```cpp\0A // Meta comment: comment within a comment\0A // ```cpp\0A // // Could be within another comment too!!!\0A // ```\0A```\0A\0ALet's see how well this will be handled!!"
// CHECK: >
// CHECK: module attributes {clift.module} {
// CHECK:   clift.func @my_commented_function<!rawfunction_0_>(
// CHECK:     !uint64_t {
// CHECK:       clift.c_attributes = [#clift.c_attribute<"_REG" : "/macro/_REG"(identifier "rax_x86_64" : "/macro-argument/_REG/rax_x86_64")>],
...
// CHECK:     },
// CHECK:     !my_struct {
// CHECK:       clift.c_attributes = [#clift.c_attribute<"_STACK" : "/macro/_STACK">],
...
!_type_definition_0_RawFunctionDefinition = !clift.func<
  ...
  [
    #clift.c_attribute<"_ABI" : "/macro/_ABI"(identifier "raw_x86_64" : "/macro-argument/_ABI/raw_x86_64")>
  ]
>
