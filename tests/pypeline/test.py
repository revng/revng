...
for param in sys.argv[1:]:
    handles.append(CDLL(param, os.RTLD_NOW | os.RTLD_LOCAL))

import revng.internal._cpp_pypeline as ext  # noqa: E402

names = [x for x in dir(ext) if not (x.startswith("__") and x.endswith("__"))]

# Check that the registries have
...
assert container.serialize({foo}) == {foo: b"foo"}

# Run the AppendFooPipe
pipe = get_registry(Pipe)["AppendFooPipe"]()
pipe.run(model, [container], [[]], [[foo]], "")
assert container.serialize({foo, bar}) == {foo: b"foofoo", bar: b"bar"}

# Run the PrintFooAnalysis
analysis = get_registry(Analysis)["AppendFooLibAnalysis"]()
analysis.run(model, [container], [[foo]], "")

yaml_model = yaml.safe_load(model.serialize())
