...
getPrototypeLayout(const model::Function &Function,
  ...
  if (const auto *CABI = Function.CABIPrototype()) {
    ...
    if (uint64_t FirstOffset = ABI.StackBytesAllocatedForRegisterArguments()) {
      ...
      // the difference here.
      if (const model::StructDefinition *Stack = Raw->stackArgumentsType())
        if (!Stack->Fields().empty())
          if (Stack->Fields().begin()->Offset() < FirstOffset)
