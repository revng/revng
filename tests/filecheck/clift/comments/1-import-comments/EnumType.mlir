...
// RUN: %root/bin/revng pipeline run-pipe import-descriptive-info %S/../0-import-types/EnumType.yml <(tar -c --transform 's;.*;/binary;' %s) /dev/stdout | %root/bin/revng clift-opt | FileCheck %s
