...
static bool
areCompatible(const model::Register &LHS, const model::Register &RHS) {
...
static StringRef areCompatible(const model::Function &Callee,
  ...
  for (const model::Register &Register : Callee.Registers) {
    auto It = CallSite.Registers.find(Register.Name);
...
void EnforceABIImpl::handleRoot() {
  ...
  for (BasicBlock &BB : *Root) {
    ...
    for (const model::Register &Register : Function->Registers) {
      if (shouldEmit(Register.Argument)) {
        GlobalVariable *CSV = M.getGlobalVariable(Register.Name, true);
...
Function *EnforceABIImpl::handleFunction(Function &OldFunction,
  ...
  for (const model::Register &Register : FunctionModel.Registers) {
    auto *CSV = cast<GlobalVariable>(M.getGlobalVariable(Register.Name, true));
...
void EnforceABIImpl::generateCall(IRBuilder<> &Builder,
  ...
  if (not IsDirect) {
    ...
    for (const model::Register &Register : CallSite.Registers) {
      GlobalVariable *CSV = M.getGlobalVariable(Register.Name, true);
      if (shouldEmit(Register.Argument)) {
        ArgumentsTypes.push_back(CSV->getType()->getPointerElementType());
        Arguments.push_back(Builder.CreateLoad(CSV));
