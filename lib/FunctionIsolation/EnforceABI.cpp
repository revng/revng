...
/// backed by a single CSV that spans exactly that size is just loaded;
/// otherwise each relevant CSV is loaded, zero-extended and shifted to its
/// position within the register, then OR'ed together.
static Value *loadRegisterFromCSVs(revng::IRBuilder &Builder,
  ...
  if (model::Register::getCSVCount(Register) == 1
      and SizeInBytes == model::Register::getSize(Register)) {
    for (model::Register::CSV &CSV : model::Register::getCSVs(Register))
      return loadCSVOrUndef(Builder, CSV);
  }

  auto *WideType = IntegerType::get(Builder.getContext(), 8 * SizeInBytes);
  ...
  for (const model::Register::CSV &CSV : model::Register::getCSVs(Register)) {
    ...
    if (CSV.StartOffset >= SizeInBytes)
      continue;
...
/// `Register`; the inverse of `assembleRegister`. Only the CSV(s) covering
/// the low `SizeInBytes` bytes are written. A single-CSV register that spans
/// exactly that size is stored directly; otherwise each relevant lane is
/// extracted (shift + truncate) and stored into its CSV.
static void serializeRegisterInCSVs(revng::IRBuilder &Builder,
  ...
  if (model::Register::getCSVCount(Register) == 1
      and SizeInBytes == model::Register::getSize(Register)) {
    for (model::Register::CSV &CSV : model::Register::getCSVs(Register)) {
      Builder.CreateStore(WideValue,
                          getCSVOrUndef(Builder.getModule(), CSV));
      return;
    }
  }

  for (const model::Register::CSV &Reg : model::Register::getCSVs(Register)) {
    // Skip the CSVs that lie entirely above the model-described prefix.
    if (Reg.StartOffset >= SizeInBytes)
      continue;

    auto *CSV = Builder.getModule()->getGlobalVariable(Reg.Name, true);
    if (CSV == nullptr)
      continue;

    Value *Slice = WideValue;
    if (Reg.StartOffset != 0)
      Slice = Builder.CreateLShr(Slice, Reg.StartOffset * 8);
    ...
    for (BasicBlock &BB : *NewFunction) {
      if (auto *Return = dyn_cast<ReturnInst>(BB.getTerminator())) {
        ...
        for (const model::Register::Portion &P : ReturnValueRegisters)
          ReturnValues.push_back(loadRegisterFromCSVs(Builder,
                                                      P.Register,
                                                      P.Size));
...
CallInst *EnforceABI::generateCall(revng::IRBuilder &Builder,
  ...
  for (const model::Register::Portion &P : Registers.Arguments)
    Arguments.push_back(loadRegisterFromCSVs(Builder, P.Register, P.Size));
  ...
  // Place the result into the CSV(s) of each return register, splitting it
