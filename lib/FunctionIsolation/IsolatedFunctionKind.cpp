    ...
    for (const model::DynamicFunction &Function : IDF) {
      ...
      if (It != OldIDF.end()) {
        if (haveFSOAndPreservedRegsChanged(getPrototype(*OldModel, *It),
          ...
          // TODO: due to a limitation in EFA, tail calls of a function that got
          // inlined are not detected. For simplicity, callers of dynamic
          // functions (ad-hoc plt trampolines created in
          // `test-invalidation.sh`) are never registered as Inline. When this
          // will be fixed, callers' attribute may be set to Inline and
          // `insertCallersAndTransitiveClosureIfInline` may be used.
          insertCallers(CrossRelations, Name, Targets);
        }
      }
    }
