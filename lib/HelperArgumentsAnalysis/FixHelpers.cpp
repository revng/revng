...
static void setMetadata(VariableManager &Variables,
  ...
  for (auto [Offset, Size] : Offsets) {
    ...
    while (RemainingSize != 0) {
      ...
      // WIP: rename functions calling this, we're putting names, not CSVs (?)
...
private:
  ...
  CallInst *emitAbort(IRBuilder<> &Builder) {
...
// WIP: move to its own file
class PurgeNonInlineFunctions : public llvm::ModulePass {
public:
  static char ID;

public:
  PurgeNonInlineFunctions() : llvm::ModulePass(ID) {}
  ...
  bool runOnModule(llvm::Module &M) override {
    ...
    // WIP: move to standalone pass?
    for (GlobalVariable &GV : M.globals()) {
      // WIP: this sucks
      if (not GV.isDeclaration() and GV.getName() != "cpu_loop_exiting"
          and GV.getName() != "tmp" and not GV.hasMetadata("revng.tags"))
        GV.setLinkage(llvm::GlobalValue::InternalLinkage);
    }

    // Assume the module M already contains the functions.
    // Collect all functions whose name starts with "helper_"
    ...
    PointerType *FuncPtrTy = PointerType::get(Context, 0);

    // Create array type: [N x <function pointer type>]
    ArrayType *ArrayTy = ArrayType::get(FuncPtrTy, Helpers.size());

    // Create constant array initializer with function pointers
    std::vector<Constant *> FunctionPointers;
    for (Function *F : Helpers) {
      FunctionPointers.push_back(ConstantExpr::getBitCast(F, FuncPtrTy));
    }
    Constant *Initializer = ConstantArray::get(ArrayTy, FunctionPointers);
...
char PurgeNonInlineFunctions::ID = 0;
using Register2 = RegisterPass<PurgeNonInlineFunctions>;
static Register2 X2("purge-non-inline-functions",
                    "Purge the body of non-inline functions",
                    true,
                    true);
