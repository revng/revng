...
static cl::opt<std::string> ModuleDumpPath("cpu-state-usage-analysis-output",
                                           cl::desc("path where to save the "
                                                    "module annotated with the "
                                                    "results of CPU State "
...
public:
  ...
  bool runOnModule(Module &M) override {
    ...
    for (auto &&Name : NoOpFunctions) {
      if (Function *F = M.getFunction(Name)) {
        auto *ReturnType = F->getFunctionType()->getReturnType();
        revng_assert(ReturnType->isVoidTy() or ReturnType->isIntegerTy());
    ...
    // NOTE: patch these things on the QEMU side wherever possible!
    std::array AbortFunctions = {
      // glib features we don't support
      "g_hash_table_lookup",
      "g_string_free",
    };
    for (auto &&Name : AbortFunctions) {
      if (Function *F = M.getFunction(Name)) {
        F->setDoesNotReturn();
        auto *Block = BasicBlock::Create(LLVMContext, "", F);
