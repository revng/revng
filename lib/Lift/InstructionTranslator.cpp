...
static unsigned getRegisterSize(const LibTcgInterface &LibTcg,
                                LibTcgOpcode Opcode) {
  switch (Opcode) {
  case LIBTCG_op_add2_i32:
  case LIBTCG_op_add_i32:
...
CallInst *
IT::emitNewPCCall(IRBuilder<> &Builder, MetaAddress PC, uint64_t Size) const {
...
IT::TranslationResult IT::translateCall(LibTcgInstruction *Instr,
  ...
  case 1: {
    ...
    Value *ResultDestination = Variables.getOrCreate(&Instr->output_args[0]);
  ...
  default: {
    ...
    revng_assert(ReturnStruct->getNumElements() == Instr->nb_oargs);

    for (unsigned I = 0; I < Instr->nb_oargs; ++I) {
      Value *ResultDestination = Variables.getOrCreate(&Instr->output_args[I]);
      revng_assert(ResultDestination != nullptr);
