    ...
    case llvm::Type::TypeID::ArrayTyID: {
      ...
      revng_log(Log,
                Prefix << "Element " << Index
                       << " in an array of " << ElementsCount << " elements. Offset in Element: " << Offset);
    } break;
...
// WIP: renmae
// WIP: make method?
static std::optional<std::pair<GlobalVariable *, unsigned>>
lol(const VariableManager::GlobalsMap &CPUStateGlobals, intptr_t Offset) {
...
VariableManager::getByCPUStateOffsetWithRemainder(intptr_t Offset) {
  ...
  // Check if we already create a variable for this offset
  if (auto MaybeResult = lol(CPUStateGlobals, Offset))
