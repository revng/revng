...
void CodeGenerator::translate(const LibTcgInterface &LibTcg,
  ...
  //
  LibTcgContext *LibTcgContext = LibTcg.context_create();
  ...
  //       struct.
  auto *ArchCPUStruct = StructType::getTypeByName(TheModule->getContext(),
                                                  "struct.ArchCPU");
  ...
  VariableManager Variables(*TheModule,
                            ...
                            ArchCPUStruct,
                            ArchInfo.env_offset,
                            LibTcg.env_ptr(LibTcgContext));

  fixHelpers(Variables, *TheModule);
  ...
  auto Factory = [&Variables,
                  &ArchInfo](PCAffectingCSV::Values CSVID,
  ...
  while (Entry != nullptr) {
    ...
    if (LibTcgLog.isEnabled()) {
      ...
      for (size_t I = 0; I < TB.instruction_count; ++I) {
        bool IsInstructionStart = TB.list[I].opcode == LIBTCG_op_insn_start;
