...
void zstdCompress(llvm::raw_ostream &OS,
  ...
  RC = ZSTD_freeCCtx(Ctx);
  revng_assert(ZSTD_isError(RC) == 0);
}

void zstdDecompress(llvm::raw_ostream &OS, DataBuffer InputBuffer) {
...
void ZstdCompressedOstream::write_impl(const char *Ptr, size_t Size) {
  zstdReadInput<ZSTDCompress>(Ctx, OS, { Ptr, Size }, OutBuffer);
}
