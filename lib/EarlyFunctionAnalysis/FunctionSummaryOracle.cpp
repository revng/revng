...
PrototypeImporter::prototype(const AttributesSet &Attributes,
  ...
  auto ToVar = std::views::transform([this](model::Register::CSV const &CSV) {
    return M.getGlobalVariable(CSV.Name, true);
  });
  auto IgnoreNullptr = std::views::filter([](const auto *Pointer) -> bool {
    return Pointer != nullptr;
  });
  for (auto Register : abi::FunctionType::calleeSavedRegisters(*Prototype))
    for (auto *CSV : model::Register::getCSVs(Register) | ToVar | IgnoreNullptr)
