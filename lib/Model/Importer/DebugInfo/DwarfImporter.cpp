...
private:
  ...
  resolveTypeWithIdentity(const DWARFDie &Die, model::UpcastableType &Type) {
    ...
    revng_assert(Definition != nullptr);
    ...
    case llvm::dwarf::DW_TAG_volatile_type: {
      ...
      // revng_assert(Typedef.UnderlyingType()->verify(true));
      // TODO: decide whether we want this assert.
    ...
    for (const auto &CU : DICtx.compile_units()) {
      for (const auto &Entry : CU->dies()) {
        ...
        if (LowPC.isValid()) {
          ...
          if (not Prototype.empty() && not Function.prototype())
          ...
          const model::TypeDefinition &Definition = Prototype->asDefinition();
