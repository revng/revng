...
private:
  ...
  std::optional<ptml::CTokenEmitter::Region> commentableTypeGuard() {
  ...
  commentableParameterGuard(DeclaratorInfo const *Declarator, uint64_t Index) {
  ...
  RecursiveCoroutine<void> emitImpl(ValueType Type,
    ...
    if (Declarator and Declarator->Kind == CTE::EntityKind::Function) {
      if (auto Function = mlir::dyn_cast<FunctionType>(Type)) {
        Parent.emitCAttributes(Function.getAttributes(),
                               /* SpaceBefore = */ false,
                               /* SpaceAfter = */ true);
    ...
    {
      auto Guard = commentableTypeGuard();
      ...
      case StackItemKind::Function: {
        ...
        if (F.getArgumentTypes().empty()) {
          ...
          for (auto [J, PT] : llvm::enumerate(F.getArgumentTypes())) {
            ...
            auto Guard = commentableParameterGuard(Declarator, J);
        ...
        if constexpr (std::same_as<std::decay_t<Type>,
          ...
          Tokens.emitIdentifier(Value.getName(),
                                ...
                                CTE::IdentifierKind::Reference);
                                          ...
                                          mlir::IntegerAttr>) {
          Tokens.emitUntypedIntegerLiteral(Value.getValue().getZExtValue());
