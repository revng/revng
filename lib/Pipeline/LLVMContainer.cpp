...
LLVMContainer::cloneFiltered(const TargetsList &Targets) const {
  ...
  auto ToCloneUntracted = InspectorT::untrackedFunctions(*this->self());

  const auto Filter = [&ToClone, &ToCloneUntracted](const auto &GlobalSym) {
    if (not llvm::isa<llvm::Function>(GlobalSym))
      return true;

    const auto &F = llvm::cast<llvm::Function>(GlobalSym);
    return ToClone.contains(F) or ToCloneUntracted.contains(F);
  };
