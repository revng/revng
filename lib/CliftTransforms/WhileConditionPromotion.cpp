...
struct WhileConditionPromotionPattern : mlir::OpRewritePattern<WhileOp> {
  ...
  matchAndRewrite(WhileOp While,
    ...
    Rewriter.eraseBlock(&While.getCondition().front());
    ...
    mlir::Region &ContinueRegion = ThenBreak ? If.getElse() : If.getThen();
    if (not isEmptyRegionOrBlock(ContinueRegion)) {
      // Hoist the continue region into the loop body.
      inlineBlockBefore(Rewriter,
                        &ContinueRegion.front(),
                        If->getBlock(),
