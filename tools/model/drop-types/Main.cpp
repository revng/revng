...
int main(int Argc, char *Argv[]) {
  ...
  for (const std::string &KeyString : TypeKeys) {
    ...
    if (SplitPoint == std::string::npos) {
      llvm::errs() << "Invalid type key (expected ID-Kind): "
                    << KeyString << "\n";
      return EXIT_FAILURE;
    }
    ...
    if (llvm::StringRef(KeyString).substr(0, SplitPoint).getAsInteger(10, ID)) {
      llvm::errs() << "Invalid ID in type key: " << KeyString << "\n";
      return EXIT_FAILURE;
    }
    ...
    if (Kind == model::TypeDefinitionKind::Invalid) {
      llvm::errs() << "Unknown type kind: " << KindName << "\n";
      return EXIT_FAILURE;
    }
    ...
    if (!Found) {
      llvm::errs() << "Type not found in model: " << KeyString << "\n";
      return EXIT_FAILURE;
    }
  }

  unsigned Dropped = model::dropTypesDependingOnDefinitions(Model,
                                                            TypesToDrop);

  llvm::errs() << "Dropped " << Dropped << " type(s)\n";
