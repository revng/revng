...
def process_graph(path, call_arcs):
    ...
    extremal_lattice_element = [
        v for v, v_data in lattice.nodes(data=True) if "peripheries" in v_data
    ][0]
...
def main():
  ...
  files representing a monotone framework."
    ...
    for path in args.inputs:
        generated_code = process_graph(path, args.call_arcs)
        all_transfer_functions |= set(generated_code["transfer_function_names"])

    monotone_framework.out(
        template.replace("%LatticeName%", generated_code["lattice_name"])
