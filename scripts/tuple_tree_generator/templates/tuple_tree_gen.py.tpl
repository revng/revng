...
def next_id():
    if not hasattr(next_id, 'next_index'):
        next_id.next_index = 0

    # TODO: automatically advance `nextIndex` if bigger values are already
    # present in this tuple tree.

    result = next_id.next_index
    next_id.next_index = next_id.next_index + 1
    return result
...
class #{ struct.name }#(
    ...
    #{- field.name }#: "#{ field | python_type }#" = field(
        ...
        ## if field.is_global_index ##default_factory=next_id,## endif ##
