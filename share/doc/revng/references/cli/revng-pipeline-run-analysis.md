`revng-pipeline-run-analysis`
================

NAME
----

`revng pipeline run-analysis` - Run a single analysis.

SYNOPSIS
--------

    revng pipeline run-analysis ANALYSIS MODEL [CONTAINER...] [OPTIONS] [-- PIPEBOX ARGS...]
    revng pipeline run-analysis-native ANALYSIS [OPTIONS] -- NATIVE RUNNER ARGS...
...
`revng pipeline run-analysis-native ANALYSIS` runs the same analysis through its native (C++) implementation, executing `libexec/revng/pypeline-run-analysis` directly without going through Python. Only analyses that have a native implementation are available.
...
```{bash notest}
revng pipeline run-analysis detect-abi revng.yml llvm-root.tar -o revng.yml
```
