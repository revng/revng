...
* **Interactivity**: rev.ng is structured to automatically infer as much information as possible from the binary, but lets the user customize it by simply editing a YAML file, aka [*the model*](user-manual/key-concepts/model.md).
  ...
  We offer [Python](user-manual/python-scripting.md) and TypeScript wrappers to interact with rev.ng.
* **Focus on data structures**: the most innovative feature of rev.ng is its focus on data structures. Thanks to [Data Layout Analysis](references/analyses.md#analyze-data-layout-analysis), we can automatically infer data structures by analyzing how a pointer is used by one or more functions.
* **Compatibility**: rev.ng can import from IDA's `.idb`/`.i64`, Linux/macOS debug information (DWARF) and Windows debug information (PDB); see the [`parse-binary` analysis](references/analyses.md#parse-binary-analysis).

In terms of design choices, its distinctive characteristics are:

* **Openness**: the entire decompilation pipeline is open source.
* **Based on QEMU**: it employs [QEMU](developer-manual/qemu-helpers.md) to lift assembly code to an architecture-independent representation.
